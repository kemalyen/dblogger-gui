@extends('dblogger-gui::layout.master')
         ...
         <select name="message" id="message"  class="form-control">
            <option value=""> - - - - </option>
             @foreach($messages as $m)
                @if($m == $message)
                <option selected value="{{ $m }}">{{ isset($messages[$m]) ? $messages[$m] : $m }}</option>
                @else
                <option value="{{ $m }}">{{ isset($messages[$m]) ? $messages[$m] : $m }}</option>
                @endif
             @endforeach
         </select>
     </div>
...
@foreach($logs as $log)
    <div class="row">
        <div class="col-xs-12">
            <div class="vleft">
                <span class="label label-default">{{ $log->level_name }}</span></td>
            </div>
            <div class="vright">
                <strong>{{ isset($messages[$log->message]) ? $messages[$log->message] : $log->message }}</strong>
                <pre>{{ $log->context }}</pre>
                <div class="subtitle">
